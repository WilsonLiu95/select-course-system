...
class CommonCourse extends Controller
{
    use BaseTrait;
    private $account;
    private $hash;
    ...
    public function getCanSelectCourse(){
        $data = $this->makeInitPageData($this->account['institute_id'], 0); // 公选课方向id为0
    ...
    private function filterCanQueueCourse($queue_course, $isQuit){
        // 将已选课程以及不可选的课程排除在外
        $has_select_common_course = collect($this->getSessionInfo('has_select_common_course'));
        $common_course = collect($this->cacheDirection($this->account['institute_id'], 0));
        $filtered = collect($queue_course)->filter(function($item)use($has_select_common_course, $common_course, $isQuit){
            // 首先课程需要在与专业方向一致。理论上,不应该出现如此情况,以防万一(用户乱伪造请求)
            if(!$common_course->contains("id", $item)){
                return false;
            }
            // 其次,需要查看已选课程
            if($has_select_common_course->contains($item)){
    ...
    public function getSelectResult(){
        ...
        if(session()->get('isQuit')){
            ...
            // 选课
            session()->put('has_select_common_course', array_merge($has_select, $success_handle)); // 更新session中,用户选中的课程
        }
