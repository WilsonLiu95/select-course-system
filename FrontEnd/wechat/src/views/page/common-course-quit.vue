<template>
  <div class="course-select-page">
    ...
    <mt-checklist id="course-check-item" v-if="canSelectCourseOptions.length" title="公共选修课程列表" v-model="finalCourseArr" :options="canSelectCourseOptions"></mt-checklist>
    ...
    methods: {
      ...
      confirm() {
        util.box.confirm("确定退选选中的方向？").then(action => {
          this.$http.post("common-course/handle-course", { course_id_arr: this.finalCourseArr, isQuit:true},{noIndicator: true}) 
          this.$indicator.open({
            text: '紧急退选ing~',
