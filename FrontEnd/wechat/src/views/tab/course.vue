<template>
  <div class="tab-page-container tab-course">
    <div class="nav tab-line">
      <mt-button class="t-btn btn"
                 type="primary"
                 @click.native="jumpPage(-1)"
                 size="small">
        上个方向
      </mt-button>
      <mt-button class="t-center-btn btn"
                 type="primary"
                 @click.native="isPopupVisible = true"
                 size="small"
                 v-if="isInit">{{current_page}}/{{direction.length}} {{direction[current_page-1]}}</mt-button>
      <mt-button class="t-btn btn"
                 type="primary"
                 @click.native="jumpPage(+1)"
                 size="small">
    ...
    <!--start 课表清单-->
    <div v-if="isInit">
      <mt-cell v-if="current_page==1"
               style="text-align:center"
               label="提示：部分公共选修课程同时属于专业方向选修课。">
      </mt-cell>
      <mt-cell v-for="(item,index) in course[current_page-1].course"
               :title="item.title"
               :label="getLabel(item)"
               :to="'/details/' + (current_page-1) +'/'+ index"
               is-link>
        {{ item.teacher}}
      </mt-cell>
    </div>
    <!--end 课表清单-->
    <!--start 选页面弹窗-->
    <mt-popup v-model="isPopupVisible"
              position="bottom"
              class="mint-popup"
              v-if="isInit">
